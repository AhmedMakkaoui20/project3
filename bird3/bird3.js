...
function initializeBoard() {
    ...
    for (let r = 0; r < numRows; r++) {
        for (let c = 0; c < numColumns; c++) {
            ...
            // Create number element and set its properties
            ...
            // Adjust margin for tiles in rows other than the first one
            if (r > 0) {
                tileContainer.style.marginTop = "-4px";
            }
            // Append tile and number elements to the tile container and add to the board
            tileContainer.appendChild(tile);
            tileContainer.appendChild(numberElement);
            document.getElementById("board").append(tileContainer);
            // Set the empty cell to the tile with "9.png"
            if (tile.src.includes("9.png")) {
                emptyCell = tile;
            }
        }
...
function isAdjacentToEmpty(tile) {
    // Get coordinates of the tile and empty cell
...
// Function to shuffle the tiles around the empty cell
